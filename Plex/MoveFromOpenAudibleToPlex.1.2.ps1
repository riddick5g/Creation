...
foreach ($m4bFile in $m4bFiles) {
    try {
        ...
        if (Test-Path $m4bDestination) {
            # Compare file sizes to ensure they're the same
            $sourceSize = $m4bFile.Length
            $destSize = (Get-Item $m4bDestination).Length
            
            if ($sourceSize -eq $destSize) {
                Write-Log "Skipping (already exists): $baseName"
                $skippedCount++
                continue
            } else {
                Write-Log "Processing (file size mismatch): $baseName"
            }
        } else {
            Write-Log "Processing (new): $baseName"
        }
